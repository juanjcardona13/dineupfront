<template>
  <CRUDDALS 
    :header="header" 
    :Create="Create"
    :Read="Read"
    :Update="Update"
    :Delete="Delete"
    :Deactivate="Deactivate"
    :Activate="Activate"
    :List="List"
    :Search="Search"
    >
  </CRUDDALS>
</template>

<script>
import ComponentFormCreateUpdateMenu from "~/components/dineUp/menu/Menu/createUpdate.vue"
import ComponentVerDetalleMenu from "@/components/dineUp/menu/Menu/read.vue"
import { useAppBusinessLogicStore } from "~/stores/app-business-logic";


const FormCreateUpdateMenu = shallowRef(ComponentFormCreateUpdateMenu);
const VerDetalleMenu = shallowRef(ComponentVerDetalleMenu);


definePageMeta({
  middleware: "middleware-auth",
})

export default {
  data() {
    return {
      header: {
        title: "Menús",
      }, 
      Create: {
        modalTitle: "Crear nuevo menú",
        objectData: {},
        showFields: {
          restaurant: false,
          branches: false
        },
        component: FormCreateUpdateMenu,
        onConfirmCreate: this.createObject,
      },
      Read: {
        modalTitle: "Detalle del menú",
        objectData: {},
        component: VerDetalleMenu,
        onOpenRead: this.readObject
      },
      Update: {
        modalTitle: "Actualizar menú",
        objectData: {},
        showFields: {
          restaurant: false,
          branches: false
        },
        component: FormCreateUpdateMenu,
        onOpenUpdate: this.openUpdateObject,
        onConfirmUpdate: this.updateObject,
      },
      Delete: {
        modalTitle: "Eliminar menú",
        objectData: {},
        onConfirmDelete: this.deleteObject,
      },
      Deactivate: {
        modalTitle: "Desactivar menú",
        objectData: {},
        onConfirmDeactivate: this.deactivateObject,
      },
      Activate: {
        modalTitle: "Activar menú",
        objectData: {},
        onConfirmActivate: this.activateObject,
      },
      List: {
        columns: [
          { label: 'Nombre', field: 'name'},
          { label: 'Acciones', field: 'actions'},
        ],
        rows: [],
        totalRows: 0,
        rowStyleClass: "selectable-row",
        paginationOptions: {
          setCurrentPage: 1,
          perPage: 10,
        },
      },
      Search: {
        enabled: true,
        valueToSearch: undefined,
      }    
    }
  },
  methods: {
    async openUpdateObject(propsRow) {
      const { data } = await this.$apollo.query({
        fetchPolicy: "no-cache",
        query: readMenu(`
          id
          name
          categories {
            id
          }
        `),
        variables: {
          where: {
            id: {
              exact: propsRow.row.id
            }
          }
        }
      })
      let objPreData = JSON.parse(JSON.stringify(data.readMenu))
      objPreData.categories = objPreData.categories.map((category) => category.id)
      this.Update.objectData = objPreData
    },
    async createObject(objectData) {
      if (!objectData.restaurant) {
        const businessLogicStore = useAppBusinessLogicStore()
        const globalRestaurant = businessLogicStore.getGlobalRestaurant
        if (globalRestaurant?.id) {
          objectData.restaurant = globalRestaurant.id
        }
      }
      await executeMutation(createMenus(), { input: [objectData] })
      this.$apollo.queries.searchMenus.refetch()
    },
    async readObject(propsRow) {
      const { data } = await this.$apollo.query({
        fetchPolicy: "no-cache",
        query: readMenu(`
          id
          name
          categories {
            id
          }
        `),
        variables: {
          where: {
            id: {
              exact: propsRow.row.id
            }
          }
        }
      })
      this.Read.objectData = data.readMenu
    },
    async updateObject(objectData) {
      await this.$apollo.mutate({
        mutation: updateMenus(),
        variables: {
          input: [objectData]
        }
      })
      this.$apollo.queries.searchMenus.refetch()
    },
    async deleteObject(objectData) {
      await this.$apollo.mutate({
        mutation: deleteMenus(),
        variables: {
          where: {
            id: {
              exact: objectData.id
            }
          }
        }
      })
      this.$apollo.queries.searchMenus.refetch()
    },
    async deactivateObject(objectData) {
      await this.$apollo.mutate({
        mutation: deactivateMenus(),
        variables: {
          where: {
            id: {
              exact: objectData.id
            }
          }
        }
      })
      this.$apollo.queries.searchMenus.refetch()
    },
    async activateObject(objectData) {
      await this.$apollo.mutate({
        mutation: activateMenus(),
        variables: {
          where: {
            id: {
              exact: objectData.id
            }
          }
        }
      })
      this.$apollo.queries.searchMenus.refetch()
    },
  },
  apollo: {
    searchMenus: {
      query: searchMenus(`
        id
        name
        isActive
      `),
      variables() {
        const businessLogicStore = useAppBusinessLogicStore()
        const globalRestaurant = businessLogicStore.getGlobalRestaurant
        let advancedSearch = this.Search.valueToSearch ? [
            {
              fieldsAndFilters: [
                {field: "name", filter: "icontains"}
              ],
              searchValues: [
                this.Search.valueToSearch
              ]
            }
          ] : undefined;
        return {
          page: this.List.paginationOptions.setCurrentPage,
          pageSize: this.List.paginationOptions.perPage,
          fieldsOfModel: {
            restaurant: globalRestaurant?.id
          },
          advancedSearch
        }
      },
      result(response) {
        let { data } = response
        let { objects } = data.searchMenus
        this.List.totalRows = data.searchMenus.total
        this.List.rows = objects
      }
    }
  }
}
</script>

<style>
.selectable-row {
  cursor: pointer;
}
.selectable-row:hover {
  background-color: #eee;
}
</style>